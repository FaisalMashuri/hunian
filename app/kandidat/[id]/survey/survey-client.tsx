  ...
  const save = () =>
    start(async () => {
      ...
      if (res.ok) {
        ...
        else router.push(`/kandidat/${initial.id}`);
  ...
  return (
    <div className="min-h-screen bg-[#F4F3F0] pb-24">
      <nav className="sticky top-0 z-50 flex h-[54px] items-center gap-3.5 border-b border-[#E4E3DF] bg-white/95 px-4 backdrop-blur sm:px-6">
        <Link href={`/kandidat/${initial.id}`} className="inline-flex shrink-0 items-center gap-1.5 rounded-lg border border-[#E4E3DF] px-3 py-1.5 text-[13px] font-medium text-zinc-500 transition-colors hover:bg-[#F4F3F0] hover:text-zinc-900">
      ...
      <div className="mx-auto max-w-[720px] px-4 py-7 sm:px-6">
        ...
        <p className="mb-5 text-[13.5px] text-zinc-500">Isi dari kunjungan langsung — melengkapi data yang belum ada <strong>dan</strong> menilai kondisi. Skor dihitung ulang otomatis (menambah dimensi Kondisi &amp; Owner).</p>
        ...
        <div className="overflow-hidden rounded-2xl border border-[#E4E3DF] bg-white shadow-sm">
          <div className="flex items-center gap-2.5 border-b border-[#E4E3DF] bg-[#FAFAF9] px-5 py-4">
            ...
            <div>
              ...
              <div className="text-xs text-zinc-500">Beri bintang &amp; tag per aspek → menentukan dimensi Kondisi &amp; Owner</div>
            </div>
          </div>
        ...
        {warning && (
          <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
            ...
            <button onClick={() => router.push(`/kandidat/${initial.id}`)} className="mt-2 block font-semibold text-amber-900 underline">Lihat kandidat →</button>
          </div>
        )}
      </div>
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 flex items-center gap-2.5 border-t border-[#E4E3DF] bg-white/95 px-4 py-3 backdrop-blur sm:px-6">
        <div className="min-w-0 flex-1">
          ...
          <div className="truncate text-xs text-zinc-500">Menambah dimensi Kondisi &amp; Owner + lengkapi data</div>
        </div>
        <Link href={`/kandidat/${initial.id}`} className="shrink-0 rounded-[10px] border-[1.5px] border-[#E4E3DF] px-4 py-2.5 text-[13.5px] font-medium text-zinc-600 transition-colors hover:bg-[#F4F3F0]">Batal</Link>
