  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-[#E4E3DF] bg-white shadow-sm">
      ...
      <div className="max-h-[300px] overflow-y-auto">
        {visible.map((p) => {
          const tone = distTone(p.distanceKm);
          return (
            <button
              key={p.id}
              type="button"
            ...
            >
              ...
              <span className="shrink-0 text-right">
                <span className={`block text-[13px] font-semibold tabular-nums ${tone.text}`}>{km(p.distanceKm)} km</span>
      ...
      <div className="border-t border-[#E4E3DF] bg-[#FAFAF9] px-4 py-2 text-[11px] text-zinc-400">
        Jarak = garis lurus (estimasi), bukan jarak tempuh jalan. Data dari OpenStreetMap.
      </div>
    </div>
  );
}
