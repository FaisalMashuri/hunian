  ...
  if (listView) {
    return (
      <div
        ...
        tabIndex={0}
        onClick={navigate}
        onKeyDown={(e) => { if (e.key === "Enter") router.push(`/kandidat/${c.id}`); }}
      ...
      >
        ...
        <button type="button" data-stop onClick={(e) => { e.stopPropagation(); router.push(`/kandidat/${c.id}`); }} className="shrink-0 rounded-[6px] bg-teal-700 px-[10px] py-[5px] text-[11.5px] font-semibold text-white hover:bg-teal-800">Detail →</button>
      </div>
    );
  }
      ...
      <div className="flex items-center gap-[6px] border-t border-[#E4E3DF] px-[13px] py-[9px]">
        ...
        <div className="ml-auto flex gap-[4px]" data-stop>
          {archived ? (
            ...
            <button type="button" onClick={(e) => { e.stopPropagation(); router.push(`/kandidat/${c.id}/edit`); }} className="rounded-[6px] border border-[#E4E3DF] px-[10px] py-[4px] text-[11.5px] font-medium text-zinc-600 transition-colors hover:bg-[#F4F3F0]">Lengkapi</button>
          ) : (
          ...
          <button type="button" onClick={(e) => { e.stopPropagation(); router.push(`/kandidat/${c.id}`); }} className="rounded-[6px] bg-teal-700 px-[10px] py-[4px] text-[11.5px] font-semibold text-white transition-colors hover:bg-teal-800">Detail →</button>
        </div>
      </div>
    </div>
  );
