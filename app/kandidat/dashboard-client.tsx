...
export function DashboardClient({
  ...
  const shown = useMemo(() => {
    ...
    out = [...out];
    if (sort === "skor") out.sort((a, b) => (b.score_total ?? -1) - (a.score_total ?? -1));
  ...
  const STATS = [
    { v: counts.aktif, label: "Kandidat aktif", cls: "text-teal-700", hint: "" },
      ...
      <div className="px-4 pb-24 pt-4 sm:px-6 sm:pt-5">
        ...
        <div className="mb-3.5 flex flex-wrap items-center gap-2.5">
          <div className="relative w-full sm:w-auto sm:min-w-[160px] sm:max-w-[280px] sm:flex-1">
            ...
            <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Cari nama kandidat..." className="h-[34px] w-full rounded-[9px] border border-[#E4E3DF] bg-white pl-8 pr-3 text-[13px] text-zinc-900 outline-none transition-colors placeholder:text-zinc-400 focus:border-teal-600" />
          </div>
        ...
        {selected.length === 0 && cards.length >= 2 && (
          <p className="mb-3 text-[11.5px] text-zinc-400">Centang ☑ di kartu untuk memilih 2–4 kandidat yang ingin dibandingkan.</p>
        )}

        {/* Grid */}
        {shown.length === 0 ? (
          <p className="rounded-2xl border border-dashed border-zinc-300 bg-white px-6 py-10 text-center text-sm text-zinc-500">Tidak ada kandidat yang cocok dengan filter.</p>
        ) : (
          <div className={grid}>
            {shown.map((c) => (
