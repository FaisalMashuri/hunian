...
export function EditCandidate({ id, initial, propertyType, typeData: initialTypeData }: { id: string; initial: ExtractedDraft; propertyType: PropertyType; typeData: TypeData }) {
  ...
  return (
    <div className="min-h-screen bg-[#F4F3F0] pb-24">
      ...
      <nav className="sticky top-9 z-50 flex h-[54px] items-center gap-3.5 border-b border-[#E4E3DF] bg-white/95 px-4 backdrop-blur sm:px-6">
        <Link href={`/shortlist/${id}`} className="inline-flex shrink-0 items-center gap-1.5 rounded-lg border border-[#E4E3DF] px-3 py-1.5 text-[13px] font-medium text-zinc-500 transition-colors hover:bg-[#F4F3F0] hover:text-zinc-900">
        ...
        <span className="min-w-0 flex-1 truncate text-[15px] font-bold tracking-tight text-zinc-900">Edit Hunian</span>
      </nav>

      <div className="mx-auto max-w-[720px] px-4 py-7 sm:px-6">
        <h1 className="mb-1.5 text-[22px] font-extrabold tracking-tight text-zinc-900">Edit Hunian</h1>
        <p className="mb-5 text-[13.5px] text-zinc-500">Perbarui data hunian. Skor &amp; jarak dihitung ulang otomatis setelah simpan.</p>
        ...
        {warning && (
          <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
            ...
            <button onClick={() => router.push(`/shortlist/${id}`)} className="mt-2 block font-semibold text-amber-900 underline">Lihat hunian →</button>
          </div>
        )}
      </div>
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 flex items-center gap-2.5 border-t border-[#E4E3DF] bg-white/95 px-4 py-3 backdrop-blur sm:px-6">
        <div className="min-w-0 flex-1">
          <div className="truncate text-sm font-bold text-zinc-900">Edit hunian</div>
          <div className="truncate text-xs text-zinc-500">Skor &amp; jarak dihitung ulang setelah simpan</div>
        </div>
        <Link href={`/shortlist/${id}`} className="shrink-0 rounded-[10px] border-[1.5px] border-[#E4E3DF] px-4 py-2.5 text-[13.5px] font-medium text-zinc-600 transition-colors hover:bg-[#F4F3F0]">Batal</Link>
